...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg( # mdp.terrain_levels_vel() 的实现里使用了 base_velocity，所以这样定义名字
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.3, 0.6), lin_vel_y=(-0.25, 0.25), ang_vel_z=(-math.pi/4, math.pi/4), # heading=(-math.pi, math.pi)
        ),
...
class EventCfg:
    ...
    physics_material = EventTerm( # 摩擦力
        ...
        params={
            ...
            "static_friction_range": (0.7, 1.25),
...
class RewardsCfg:
    ...
    tracking_lin_vel = RewTerm(func=mdp.track_lin_vel_xy_exp, weight=1.8, params={"std": 0.5, "command_name": "base_velocity"})
    ...
    torques = RewTerm(func=mdp.applied_torque_limits, weight=-0.00005)
    dof_acc = RewTerm(func=mdp.joint_acc_l2, weight=-1.e-6)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.001)
    soft_dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-1.0, 
                                  params={"asset_cfg": SceneEntityCfg("robot", 
                                                                    joint_names=[".*",])})
    bad_orientation = RewTerm(func=mdp.flat_orientation, weight=-1.0)
    base_height = RewTerm(func=mdp.base_height, weight=-10., 
    ...
    feet_air_time = RewTerm(func=mdp.feet_air_time, weight=1.0, params={"command_name":"base_velocity", 
                                                                        ...
                                                                        "threshold": 0.5})
    hip_pos = RewTerm(func=mdp.joint_deviation, weight=0.1, 
                      params={"asset_cfg": SceneEntityCfg(name="robot", 
                                                          joint_names=["lf_hip_roll_joint", "lb_hip_roll_joint", "rf_hip_roll_joint", "rb_hip_roll_joint"],
                                                          preserve_order=True),})
    ...
    # lift_foot = RewTerm(func=mdp.lift_foot, weight=1.0, 
    #                     params={"command_name": "base_velocity",
    #                             "target_pos": -1.95, 
    #                             "asset_cfg": SceneEntityCfg(name="robot", 
    #                             joint_names=["lf_knee.*", "rb_knee.*", "rf_knee.*", "lb_knee.*"],
    #                             preserve_order=True)})
    # recovery_thigh = RewTerm(func=mdp.recovery_thigh, weight=-3.0, 
    #                             params={"target_pos": 1.77, "asset_cfg": SceneEntityCfg(name="robot", 
    #                                                                                     joint_names=[".*hip_pitch.*"]),})
    # feet_height = RewTerm(func=mdp.feet_height, weight=2.0, 
    #                       params={"command_name": "base_velocity", 
    ...
    #                                                                 body_names=["lf_foot_link", "rf_foot_link", "lb_foot_link", "rb_foot_link"],
    #                                                                 preserve_order=True)})
    # stand_still = RewTerm(func=mdp.stand_still, weight=-0.005, 
    #                       params={"command_name": "base_velocity", 
    #                               "command_threshold": 0.1,
                                #   "asset_cfg": SceneEntityCfg(name="robot")})
    # feet_stumble = RewTerm(func=mdp.feet_stumble, weight=-1., params={"sensor_cfg": SceneEntityCfg(name="contact_sensor"), "asset_cfg": SceneEntityCfg(name="robot", body_names=[".*foot_link", ".*calf_link"], preserve_order=True)})
    # stretch = RewTerm(func=mdp.reward_stretch, weight=0.01, params={"command_name": "base_velocity", 
    #                                                                "asset_cfg": SceneEntityCfg(name="robot", 
    #                                                                                            joint_names=["lf_knee.*", "rf_knee.*", "lb_knee.*", "rb_knee.*"], 
    #                                                                                            preserve_order=True)})
