...
class CommandsCfg:
    ...
    base_velocity = mdp.CommandCfg(
        asset_name="robot",
        resampling_time_range=(5.0, 5.0),
        rel_standing_envs=0.12,
        debug_vis=False,
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.4), lin_vel_y=(-1.0, 1.0), ang_vel_z=(-1.5, 1.5), # heading=(-math.pi, math.pi)
        ),
        probability=0.334,
    )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, noise=UniformNoise(operation="add", n_min=-0.15, n_max=0.15), scale=0.5,)
        proj_gravity = ObsTerm(func=mdp.projected_gravity, noise=UniformNoise(operation="add", n_min=-0.01, n_max=0.01), scale=1.0,)
        ...
        joint_torque = ObsTerm(func=mdp.robot_joint_torque,
                                noise=UniformNoise(operation="add", n_min=-1.6, n_max=1.6),
                                scale=0.1, 
                                params={"asset_cfg": SceneEntityCfg("robot", 
                                                                    joint_names=[*WL_DOG_LEG_JOINT_NAMES, *WL_DOG_WHEEL_JOINT_NAMES],
                                                                    preserve_order=True)})
    ...
    class CriticCfg(ObsGroup):
        ...
        robot_joint_acc = ObsTerm(func=mdp.robot_joint_acc, params={"asset_cfg": SceneEntityCfg("robot", 
                                                                            joint_names=[*WL_DOG_LEG_JOINT_NAMES, *WL_DOG_WHEEL_JOINT_NAMES],
                                                                            preserve_order=True)})
...
class RewardsCfg:
    ...
    undesired_contacts = RewTerm(func=mdp.undesired_contacts, weight=-0.1,
                                params={"threshold": 5.0,
                                        "sensor_cfg": SceneEntityCfg(name="contact_sensor",
                                                                    body_names=["base_link", ".*KENN.*"]),})
    lin_vel_z = RewTerm(func=mdp.lin_vel_z, weight=-0.4)
    ...
    soft_dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-1., 
                                  params={"asset_cfg": SceneEntityCfg("robot", 
                                                                    joint_names=["^(?!.*FOOT).*",])})
    ...
    #                                                                     "sensor_cfg": SceneEntityCfg(name="contact_sensor",
    #                                                                                                  body_names=[".*FOOT.*"]),})
    bad_orientation = RewTerm(func=mdp.flat_orientation, weight=-0.25, params={"command_name":"base_velocity"})
    hip_pos = RewTerm(func=mdp.joint_deviation, weight=-0.1, params={"asset_cfg": SceneEntityCfg(name="robot", joint_names=".*ABAD.*"), 
                                                                        "command_name": "base_velocity"})
    hip_symmetry = RewTerm(func=mdp.hip_joint_symmetry, weight=-0.04)
    pen_action_smoothness = RewTerm(func=mdp.ActionSmoothnessPenalty, weight=-0.0004)
    joint_powers_l1 = RewTerm(func=mdp.joint_powers_l1, weight=-2.5e-5, params={"asset_cfg": SceneEntityCfg(name="robot", 
                                                                                                                joint_names=["^(?!.*FOOT).*"])})
    energy_regularization = RewTerm(func=mdp.energy_regularization, weight=0.1, params={"asset_cfg": SceneEntityCfg(name="robot", 
                                                                                                                     joint_names=[".*FOOT.*"])})
    stand_still = RewTerm(func=mdp.stand_still, weight=-0.1, 
                          params={"asset_cfg": SceneEntityCfg(name="robot",joint_names=["^(?!.*FOOT).*"],)})
    # dof_vel = RewTerm(func=mdp.dof_vel, weight=-0.02, 
    #                       params={"command_name": "base_velocity", 
    #                               "command_threshold": 0.96,
    #                               "asset_cfg": SceneEntityCfg(name="robot",joint_names=[".*FOOT.*"],)})
    lift_feet = RewTerm(func=mdp.lift_when_blocked, weight=0.24, 
