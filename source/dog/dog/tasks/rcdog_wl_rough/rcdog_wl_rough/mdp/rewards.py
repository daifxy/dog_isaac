...
def energy_regularization(env: ManagerBasedRLEnv, 
                          asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
                          sigma_lin: float = 1000,
                          sigma_ang: float = 500,
                          clip_lin: float = 0.08,
                          clip_ang: float = 0.08) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    power = torch.sum(torch.abs(torch.mul(asset.data.applied_torque[:, asset_cfg.joint_ids], asset.data.joint_vel[:, asset_cfg.joint_ids])), dim=1)
    lin_vel = torch.abs(asset.data.root_com_lin_vel_b[:, 0])
    ang_vel = torch.abs(asset.data.root_com_ang_vel_b[:, 2])
    denom = (
        sigma_lin * torch.clamp(lin_vel, min=clip_lin) + sigma_ang * torch.clamp(ang_vel, min=clip_ang)
    )
...
def hip_joint_symmetry(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    # compute out of limits constraints
...
def lift_when_blocked(env: ManagerBasedRLEnv, 
    ...
    for idx, id in enumerate(sensor_cfg.body_ids):
        ...
        feet_height = (asset.data.body_pos_w[:, id, 2] - torch.mean(sensor.data.ray_hits_w[..., 2], dim=1) - 0.3).clip(min=1.e-6, max=0.40)
        quat = asset.data.body_quat_w[:, id]
        force = contact_sensor.data.net_forces_w[:, id, :]
        force_world = force_body_to_world_torch(force, quat)
