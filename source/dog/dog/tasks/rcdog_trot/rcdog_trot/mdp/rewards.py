...
def feet_stumble(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    for id in asset_cfg.body_ids:
        ...
        touch += (torch.norm(force_world[:, :2], dim=1) > 6.5).float() #* torch.abs(force_world[:, 2])).float()
    return touch * mask


def feet_stumble_lift(env: ManagerBasedRLEnv, 
    ...
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    for idx, id in enumerate(asset_cfg.body_ids):
        sensor: RayCaster = env.scene.sensors[sensors[idx]]
        feet_height = (asset.data.body_pos_w[:, asset_cfg.body_ids[idx], 2] - torch.mean(sensor.data.ray_hits_w[..., 2], dim=1) -0.02).clip(min=0., max=0.151)
        quat = asset.data.body_quat_w[:, id]
        force = contact_sensor.data.net_forces_w[:, id, :]
        force_world = force_body_to_world_torch(force, quat)
        rew += (feet_height * (torch.norm(force_world[:, :2], dim=1) > 6.5).float())
    return rew# * mask
