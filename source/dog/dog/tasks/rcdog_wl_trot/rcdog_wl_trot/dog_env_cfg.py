...
class CommandsCfg:
    ...
    base_velocity = mdp.CommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.5), lin_vel_y=(-1.0, 1.0), ang_vel_z=(-1.5, 1.5)
        ),
        probability=0.,
    )
...
class RewardsCfg:
    ...
    tracking_lin_vel = RewTerm(func=mdp.track_lin_vel_xy_exp, weight=1.2, params={"std": 0.632, "command_name": "base_velocity"})
    tracking_ang_vel = RewTerm(func=mdp.track_ang_vel_z_exp, weight=1.0, params={"std": 0.632, "command_name": "base_velocity"})
    ...
    feet_air_time = RewTerm(func=mdp.feet_air_time, weight=1.0, params={"command_name": "base_velocity",
                                                                        ...
                                                                        "sensor_cfg": SceneEntityCfg(name="contact_sensor",
                                                                                                     body_names=[".*FOOT.*"]),})
    bad_orientation = RewTerm(func=mdp.flat_orientation, weight=-1.0, params={"command_name":"base_velocity"})
    hip_pos = RewTerm(func=mdp.joint_deviation, weight=-0.05, params={"asset_cfg": SceneEntityCfg(name="robot", joint_names=".*ABAD.*"), 
