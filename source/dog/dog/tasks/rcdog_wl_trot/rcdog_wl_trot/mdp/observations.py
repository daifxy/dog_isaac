...
def robot_height(
    ...
    if sensor_cfg is not None:
        ...
        return sensor.data.pos_w[:, 2] - sensor.data.ray_hits_w[..., 2]
    else:
        return asset.data.root_pos_w[:, 2]


def depth_camera(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor: 
    sensor: RayCaster = env.scene.sensors[sensor_cfg.name]
    return torch.norm(sensor.data.pos_w[:, :3].unsqueeze(1).expand(-1, 45, -1) - sensor.data.ray_hits_w[..., :3], dim=-1)
