...
def get_env_cfg():
    ...
    curriculum_cfg = {
        "curriculum_commands_check_interval": 1000,  #每多少step更新一次课程学习
    ...
    other_randomize_cfg = { # 手动调用，在这里配置参数
        "randomize_base_com_range":  {"x":(-0.01, 0.01), "y":(-0.01, 0.01), "z":(-0.01, 0.01)}, # 均匀采样，以`add`方式添加到初始值
        "randomize_other_com_range": {"x":(-0.005, 0.005), "y":(-0.005, 0.005), "z":(-0.005, 0.005)}, 
        ...
        "external_force_and_torque" : { # 随机施加外力
            ...
            "curriculum_min_force_proportion": 1,
        },
    ...
    env_cfg = {
        ...
        "action_space" : 12,
        "policy_slice_obs": 33,
        "observation_space" : 147, # 36*4 + last_action(12) + command(3)
        "state_space": 147,#148, # 159 + privileged_obs(1)
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "diagonal_thigh_calf_similar": -0.08,
            # "orientation": -0.1,
            "feet_air_time": 2.,
            # "stand_steadily": -10,
            # "stable_feet": 0.06,
            "undesired_contacts": -1.0,
            "joint_torques": -0.0002,
            "dof_pos_limits": -10.0,
            # "airborne_penalty": -0.5,
            # "stand_still": -0.1,
            # "joint_vel": -0., # -6e-3
            # "feet_force": -1.0,
        },
...
class MySceneCfg(InteractiveSceneCfg):
    terrain: TerrainImporterCfg = ROUGH_TERRAIN_CFG
...
@configclass
class RoughEnvCfg(DirectRLEnvCfg):
    ...
    termination_if_base_connect_plane = True
    die_if_contact_bodies = "base_link"
    undesired_contact_bodies = [".*_hip_Link", ".*_thigh_Link"]
    feet_contact_bodies = [".*_calf_Link"]
