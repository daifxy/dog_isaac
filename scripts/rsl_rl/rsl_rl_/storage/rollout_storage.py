...
from ..modules import HiddenState
...
class RolloutStorage:
    ...
    class Transition:
        ...
        def __init__(self) -> None:
            ...
            self.encoder_out: torch.Tensor | None = None
            """Encoder outputs at the current step."""
    ...
    class Batch:
        ...
        def __init__(
            ...
            self.encoder_out: torch.Tensor | None = encoder_out
    ...
    def __init__(
        ...
        if encoder_dim is not None:
            self.encoder_out = torch.zeros(num_transitions_per_env, num_envs, encoder_dim, device=self.device)
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if self.encoder_dim is not None:
            self.encoder_out[self.step].copy_(transition.encoder_out.view(-1, self.encoder_out.shape[-1]))
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator[Batch, None, None]:
        ...
        if self.encoder_dim is not None:
            encoder_out = self.encoder_out.flatten(0, 1)
        else:
            encoder_out = torch.zeros_like(actions)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield RolloutStorage.Batch(
                    ...
                    encoder_out=encoder_out[batch_idx],
    ...
    def encoder_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                labels = observations[batch_idx]["labels"]
                encoder_obs = observations[batch_idx]
                yield encoder_obs, labels