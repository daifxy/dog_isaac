...
from ..extensions import RandomNetworkDistillation, resolve_rnd_config, resolve_symmetry_config
from ..models import MLPModel
from ..storage import RolloutStorage
from ..utils import resolve_callable, resolve_obs_groups, resolve_optimizer

from ..models import EncoderMLPModel
...
class PPO:
    ...
    def __init__(
        ...
        est_learning_rate: float = 1.e-03,
    ...
    def act(self, obs: TensorDict) -> torch.Tensor:
        ...
        self.transition.actions = self.actor(obs, encoder_out, stochastic_output=True).detach()
        ...
        if self.encoder.is_mlp_encoder:
            self.transition.encoder_out = encoder_out.detach()
    ...
    def process_env_step(
        ...
        encoder_out = self.encoder(obs)
        self.actor.update_normalization(obs, encoder_out)
        ...
        if self.encoder.is_mlp_encoder:
            ...
            if self.extra_optimizer is not None:
                ...
                for (
                    ...
                    encode_batch = self.encoder(encoder_obs)
                    extra_loss = (
                        (encode_batch - labels).pow(2).mean()
                    )
...
# ------------------------------------------------------------------------------------------------------
        ...
        if num_updates_extra > 0:
            loss_dict["encoder"] = mean_extra_loss
    ...
    def save(self) -> dict:
        ...
        saved_dict = {
            ...
            "extra_optimizer_state_dict": self.extra_optimizer.state_dict() if self.extra_optimizer is not None else None,
        }
        if self.rnd:
    ...
    def get_encoder(self) -> EncoderMLPModel:
    ...
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> PPO:
        ...
        encoder_class: type[EncoderMLPModel] = resolve_callable(cfg["encoder"].pop("class_name"))  # type: ignore

        # Resolve observation groups
        default_sets = cfg["default_sets"]
        ...
        encoder: EncoderMLPModel = encoder_class(obs, cfg["obs_groups"], "encoder", **cfg["encoder"]).to(device)
        print(f"Encoder Model: {encoder}")
        actor: MLPModel = actor_class(obs, cfg["obs_groups"], "actor", env.num_actions, encoder.output_dim, **cfg["actor"]).to(device)
        ...
        critic: MLPModel = critic_class(obs, cfg["obs_groups"], "critic", 1, None, **cfg["critic"]).to(device)
        print(f"Critic Model: {critic}")

        # Initialize the storage
        storage = RolloutStorage("rl", env.num_envs, cfg["num_steps_per_env"], obs, [env.num_actions], encoder.output_dim, device)

        # Initialize the algorithm
        alg: PPO = alg_class(actor, critic, encoder, storage, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"])
