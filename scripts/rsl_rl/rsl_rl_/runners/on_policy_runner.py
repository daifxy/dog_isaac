...
from ..utils import check_nan, resolve_callable
...
class OnPolicyRunner:
    ...
    def get_inference_encoder(self, device: str | None = None) -> EncoderMLPModel:
        """Return the encoder on the requested device for inference."""
        self.alg.eval_mode()  # Switch to evaluation mode (e.g. for dropout)
        return self.alg.get_encoder().to(device)  # type: ignore
    ...
    def export_policy_to_jit(self, path: str, filename: str = "policy.pt") -> None:
        ...
        if self.alg.encoder.is_mlp_encoder:
            encoder_jit_model = self.alg.get_encoder().as_jit()
            encoder_jit_model.to("cpu")
            encoder_traced_model = torch.jit.script(encoder_jit_model)
            encoder_traced_model.save(os.path.join(path, "encoder.pt"))
    ...
    def export_policy_to_onnx(self, 
        ...
        if self.alg.encoder.is_mlp_encoder:
            encoder_onnx_model = self.alg.get_encoder().as_onnx(verbose=verbose)
            encoder_onnx_model.to("cpu")
            encoder_onnx_model.eval()
            encoder_save_path = os.path.join(path, "encoder.onnx")
            torch.onnx.export(
                encoder_onnx_model,  # type: ignore 
                encoder_onnx_model.get_dummy_inputs(),  # type: ignore
                encoder_save_path,
                export_params=export_params,
                opset_version=opset_version,
                verbose=verbose,
                input_names=encoder_onnx_model.input_names,  # type: ignore
                output_names=encoder_onnx_model.output_names,  # type: ignore
            )
