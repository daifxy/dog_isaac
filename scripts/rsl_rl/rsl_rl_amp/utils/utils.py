...
class RunningMeanStd:
    ...
    def update_from_moments(self, batch_mean: np.ndarray, batch_var: np.ndarray, batch_count: int) -> None:
        ...
        # Mark cache as dirty when stats are updated (for Normalizer subclass)
...
class Normalizer(RunningMeanStd):
    def __init__(self, input_dim, epsilon=1e-4, clip_obs=10.0):
        ...
        # Cache for torch tensors to avoid repeated conversion
    ...
    def normalize_torch(self, input, device):
        # Update cache if dirty or device changed
        if self._cache_dirty or self._cached_device != device:
            self._cached_mean_torch = torch.tensor(self.mean, device=device, dtype=torch.float32)
        ...
        return torch.clamp((input - self._cached_mean_torch) / self._cached_std_torch, -self.clip_obs, self.clip_obs)
